...
class Users(Base):
    ...
    vk_link = sqlalchemy.Column(sqlalchemy.String(length=100), nullable=False, unique=True) #я добавила
...
class UserActionsAssociation(Base):
    ...
    association_id = sqlalchemy.Column(sqlalchemy.Integer, primary_key=True)
    user_id = sqlalchemy.Column(sqlalchemy.Integer, sqlalchemy.ForeignKey("users.user_id"), nullable=False)
    action_id = sqlalchemy.Column(sqlalchemy.Integer, sqlalchemy.ForeignKey("user_actions.action_id"), nullable=False)
