...
class VK:
    ...
    def search_people(self, city, age, sex): # поменять count на 1000
        params = self.common_params()
        params.update({"user_id": self.id, "count": 10, "city": city, "age_from": age - 2, "age_to": age + 5, "sex": sex, "fields": "id, domain, city, bdate, sex"})
    ...
    def users_list(self): # список фамилий,имен, ссылок пользователей + 3 фотографии
        ...
        for match in matches_file:                
            ...
            final_file.append([name,sunname,age,sex,city,id,users_photos,link,self.param_user()])
        return final_file
