...
class Users(Base):
    ...
    actions = relationship("UserActions", secondary="user_actions_association", back_populates="users")
...
class Photos(Base):
    ...
    url_photo = sqlalchemy.Column(sqlalchemy.String(length=255), nullable=False)
...
class Favorites(Base):
    ...
    def __str__(self):
        return f"Favorite {self.favorite_id} : {self.user_id}, {self.added_user_id}, {self.favorite_insert_time}"
...
class UserActions(Base):
    ...
    users = relationship("Users", secondary="user_actions_association", back_populates="actions")

    def __str__(self):
        return f'Stock {self.id_stock} : ({self.stock_count}, {self.id_book}, {self.id_shop})'


class UserActionsAssociation(Base):
    __tablename__ = "user_actions_association"

    user_id = sqlalchemy.Column(sqlalchemy.Integer, sqlalchemy.ForeignKey("users.user_id"), primary_key=True)
    action_id = sqlalchemy.Column(sqlalchemy.Integer, sqlalchemy.ForeignKey("user_actions.action_id"), primary_key=True)
